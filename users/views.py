...
@login_required
@user_passes_test(is_admin_or_superadmin)  # Restrict access based on role
...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Check if the user role is superadmin to make them a Django superuser
            if user.role == 'superadmin':
            ...
            if user.role == 'superadmin':
                return redirect('/admin')  # Redirect superadmin directly to Django admin
            elif user.role == 'admin':
                return redirect('admin_dashboard')
            else:
                return redirect('user_dashboard')
